...
const DELIVERY_BY_BAIRRO = {
  "Ribeirão da Ilha": 3.90,
  "Carianos": 5.50,
  "Centro": 4.50,
  "Continente": 6.00,
  "tapera": 0.00
...
function getDeliveryFeeByBairro(bairro) {
  const key = normalizeBairro(bairro);
  return DELIVERY_BY_BAIRRO[key] ?? DEFAULT_DELIVERY_FEE;
}
...
const OrderSummary = ({ cart, deliveryType = 'delivery' }) => {
  ...
  ? getDeliveryFeeByBairro(customerInfo.bairro)
  : 0;
...
{showInstallBanner && (
  <div className="fixed bottom-5 left-0 right-0 z-50 flex justify-center">
    <div className="bg-white shadow-lg rounded-lg p-4 flex items-center gap-3 border">
      <div className="font-semibold text-sm">Adicionar à Tela Inicial</div>
      <button
